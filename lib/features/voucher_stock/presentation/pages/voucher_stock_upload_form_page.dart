...
class VoucherStockUploadFormPage extends ConsumerStatefulWidget {
  const VoucherStockUploadFormPage({super.key, this.showBackButton = true});

  final bool showBackButton;

  @override
  ConsumerState<VoucherStockUploadFormPage> createState() =>
      _VoucherStockUploadFormPageState();
}

class _VoucherStockUploadFormPageState
    extends ConsumerState<VoucherStockUploadFormPage> {
  ...
  Future<void> _onSimpan() async {
    ...
    try {
      await ref
          .read(digitalVoucherProvider.notifier)
          .createBulk(
      ...
      final submittedCount = _codes.length;
      if (!mounted) return;
      setState(() {
        _isSubmitting = false;
        _codes = const [];
      ...
      _showToast('$submittedCount voucher berhasil dikirim');
    } catch (e) {
      if (!mounted) return;
      setState(() => _isSubmitting = false);
      _showToast(e.toString(), destructive: true);
    }
  }
                      ...
                      children: [
                        Expanded(
                          child: districtsAsync.when(
                            ...
                            loading: () => _buildSkeletonDropdown(
                              'Kecamatan',
                              'Memuat kecamatan...',
                            ),
                            error: (e, _) => Text(
                          ...
                          Expanded(
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, i) =>
                                  VoucherCodeRow(index: i, code: _codes[i]),
                            ),
                          ),
                        ],
                      ),
  ...
  Widget _buildSkeletonDropdown(String label, String hint) {
    return Skeletonizer(
      ...
      child: FSelect<String>(
        ...
        label: Text(label),
        hint: hint,
        control: FSelectControl.lifted(value: '', onChange: (_) {}),
      ),
    );
  }
  ...
  Widget _buildVoucherDropdown(
    ...
    return vouchersAsync.when(
      ...
      data: (all) => assignedIdsAsync.when(
        loading: () =>
            _buildSkeletonDropdown('Voucher', 'Memuat voucher desa...'),
        error: (e, _) => Text(
